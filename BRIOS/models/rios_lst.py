...
def check_values(x_y, m_y):
    count = 0
    for i in range(len(x_y)):
        if x_y[i] == -100 and m_y[i] == 1:
            print(f"Chỉ số {i} có x_y[{i}] = {x_y[i]} và m_y[{i}] = {m_y[i]}")
            count += 1
...
class RIOS_H(nn.Module):
    ...
    def build(self):
        ...
        self.rnn_cell1 = nn.LSTMCell(self.SELECT_SIZE, self.rnn_hid_size)
    ...
    def forward(self, data, direct):
        ...
        for t in range(self.SEQ_LEN):
            ...
            m_y = masks[:, t]
            d_y = deltas[:, t, 1]
            # x_y = torch.nan_to_num(x_y, nan=-100)
